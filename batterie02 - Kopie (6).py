...
def schreibe_excel(aktionen, dateiname):
    ...
    for i, zeile in enumerate(aktionen):
        ...
        formatierte_zeile = [
            neue_zeile[0],
            neue_zeile[1],
        ] + [f'{x:.4f}' if isinstance(x, (int, float)) and x != 0 else '' for x in neue_zeile[2:]]
        ...
        # Wenn nächster Tag oder letzte Zeile, Tagessumme schreiben
        next_tag = None
        if i + 1 < len(aktionen):
        ...
        if next_tag != tag:
            # Summen in die richtigen Spalten, Preis und Füllstand leer
            sum_row = [tag, 'Tagessumme'] + [f'{x:.4f}' if x != 0 else '' for x in tagessummen[tag]]
            # Füge leere Felder für Preis und Füllstand ein
    ...
    for monat, summen in monatssummen.items():
        sum_row = [monat, 'Monatssumme'] + [f'{x:.4f}' if x != 0 else '' for x in summen]
        sum_row = sum_row[:5] + ['', ''] + sum_row[5:]
        ws.append(sum_row)
        for col in range(1, len(header)+1):
            ws.cell(row=ws.max_row, column=col).font = Font(bold=True)
            ws.cell(row=ws.max_row, column=col).alignment = Alignment(horizontal='center', vertical='center')
    # Gesamtsumme
    sum_row = ['Gesamt', 'Gesamtsumme'] + [f'{x:.4f}' if x != 0 else '' for x in gesamtsummen]
    sum_row = sum_row[:5] + ['', ''] + sum_row[5:]
    ws.append(sum_row)
    for col in range(1, len(header)+1):
        ws.cell(row=ws.max_row, column=col).font = Font(bold=True)
...
def main():
    ...
    schreibe_excel(aktionen, 'batterie_aktionen.xlsx')
