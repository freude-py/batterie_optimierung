...
def lade_daten(excel_datei):
    ...
    for i, row in enumerate(ws.iter_rows(min_row=2, values_only=True)):
        if i >= 100000:    # für testzwecke kann hier die Anzahl der Zeilen begrenzt werden
            break
        datum = row[0]
        try:
...
def batterie_bewirtschaftung(daten, batterie_kapazitaet=10.0, lade_menge=0.5, ladung_min=10, ladung_max=90, speicher_verluste=10, entlade_verluste=10):
    ...
    print("Starte Optimierung mit pulp. Statusmeldungen folgen alle paar Sekunden...")
    prob.solve(pulp.PULP_CBC_CMD(msg=1))                               # Lösen mit Statusausgabe
    print("Optimierung abgeschlossen, Ergebnisse werden verarbeitet...")
...
def schreibe_excel(aktionen, dateiname, parameter):
    ...
    # Automatischer Filter ab Zeile 5 (Header)
...
def main():
    ...
    dateiname = f'batterie_aktionen_{zeitstempel}.xlsx'
    schreibe_excel(aktionen, dateiname, parameter)
